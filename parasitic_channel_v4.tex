...
\author[1]{Tana Kieper\thanks{ORCID: 0009-0005-7164-0085. Correspondence: tana@protogenbio.com}}
...
\subsection{Convergent Evolution and the Game-Theoretic Bridge}
\label{sec:convergence}

Three phylogenetically unrelated lineages---\textit{Plasmodium} (Apicomplexa), \textit{Trypanosoma} (Kinetoplastida), and HIV (Retroviridae)---independently evolved antigenic repertoires whose entropy exceeds per-interaction signaling capacity.  This convergence is consistent with strong selection pressure toward the $H > C$ regime at the bottleneck level.  The mathematical connection between information theory and game theory explains why an IT framework is appropriate for host--pathogen systems---not merely analogical, but a different projection of the same underlying mathematics.

\textbf{Channel capacity as a minimax game.}  \citet{gallager1968} showed that channel capacity has the mathematical structure of a zero-sum game: the capacity formula $C = \max_{P(X)} \min_{q(S)} I(X;Y)$ from the encoder's perspective and $\min_{q(S)} \max_{P(X)} I(X;Y)$ from the adversary's perspective satisfies von Neumann's minimax theorem~\citep{vonneumann1928}.  The saddle-point solution is the Nash equilibrium of this game.  This is a proven theorem, not an analogy: channel capacity \emph{is} the value of a zero-sum game over mutual information.  The arbitrarily varying channel (AVC) specializes this to adversarial noise: \citet{csiszar1988} established that an adversary can drive deterministic-code capacity to zero if it achieves ``symmetrizability''---the ability to perfectly mimic legitimate signals.  No biological parasite achieves perfect symmetrizability, but the AVC framework establishes the \emph{direction} of selection pressure.

\textbf{Evolution as information-theoretic optimization.}  \citet{chastain2014} proved that natural selection under sexual recombination is mathematically equivalent to the Multiplicative Weight Updates Algorithm (MWUA), which maximizes cumulative expected fitness plus entropy.  Evolution is, provably, an information-theoretic optimization process.  \citet{bergstrom2005} proved the connecting result: the fitness value of environmental information equals the mutual information $I(X;Y)$ between environmental cue and state.  In our context, a parasite that reduces the mutual information between its antigens and the immune response---by increasing antigenic entropy beyond channel capacity---directly reduces the fitness value of the host's information channel.

\textbf{The bridge.}  These connections explain why applying Shannon's framework to parasitology is not a metaphorical leap.  Evolutionary game theory has modeled host--parasite coevolution for four decades~\citep{bremermann1983,nowak1994,frank1996}, producing validated predictions about virulence, transmission, and resistance.  Shannon's information theory shares identical mathematical structure with game theory at the level of proven theorems (minimax, saddle-point, MWUA).  Using IT to analyze host--pathogen dynamics is therefore using a different projection of the same mathematics---game theory asks what strategy the parasite \emph{should} adopt; IT asks what happens to the \emph{signal}.  The two frameworks ask different questions using shared mathematical structure.

We treat these connections as theoretical context, not as formal models of the biology.  No biological AVC has been instantiated with measured state spaces, costs, and symmetrizability conditions.  But the mathematical structure explains why an information-theoretic framework is \emph{natural} for adversarial biological systems, and why the convergence of three unrelated lineages on $H > C$ is expected under the selection pressure that these theorems describe.

Many parasites adopt strategies other than antigenic variation---intracellular hiding, rapid transmission before clearance, direct immunosuppression---that do not require large antigenic repertoires.  The claim is narrower than ``all parasites must evolve $H > C$'': antigenic variation, \emph{when it evolves}, faces selection pressure toward entropy levels that exceed per-interaction signaling capacity.  The three lineages we study are the canonical examples of this strategy, and all three exceed the bottleneck benchmark.
