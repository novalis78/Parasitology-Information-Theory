...
\item Apply Miller-Madow finite-sample bias correction~\citep{miller1955}: $H_{\text{corrected}} = H_{\text{raw}} - (K-1)/(2N\ln 2)$, where $K = |\mathcal{X}|$ and $N$ is the number of observations.  (The naive plugin entropy estimator has a known positive bias of approximately $(K-1)/(2N\ln 2)$; we subtract this correction following \citealt{miller1955}.)
