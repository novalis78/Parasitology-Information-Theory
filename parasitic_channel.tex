...
\end{equation}
Even with $10^6$ coordinated cells, the minimum error probability for discriminating HIV 9-mer epitopes exceeds 60\%.  Third, the \textit{T.~brucei} oscillation data provides direct empirical evidence that the capacity constraint binds \emph{in vivo}---clearance occurs only when entropy drops below ${\sim}1$~bit, consistent with a per-interaction capacity constraint.
